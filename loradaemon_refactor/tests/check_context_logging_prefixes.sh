...
require() {
  ...
  if ! grep -Fq -- "$pattern" "$DAEMON" && ! grep -Fq -- "$pattern" "$LOG_H" && ! grep -Fq -- "$pattern" "$LOG_CPP"; then
...
require "void daemon_verbose_ctx(const char *ctx, const char *fmt, ...)" "verbose context helper"
require "void daemon_debug_ctx(const char *ctx, const char *fmt, ...)" "debug context helper"
